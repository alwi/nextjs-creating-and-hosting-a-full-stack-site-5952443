...
export default function ProductsList({ products, initialCartProducts = [] }: { products: Product[], initialCartProducts: Product[] }) {
  const [productListCartProducts, setProductListCartProducts] = useState(initialCartProducts)
  ...
  async function addToCart(productId: string) {
    ...
    setProductListCartProducts(updatedCartProducts);
  }
  ...
  function productIsInCart(productId: string) {
    return productListCartProducts.some(cp => cp.id === productId);
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
      {products.map(product => (
        ...
        >
          ...
          <h2 className="text-xl font-semibold mb-2 text-gray-700">{product.name}</h2>
